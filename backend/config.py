...
LLM_MIN_INTERVAL = _i("LLM_MIN_INTERVAL", 180)
LLM_MAX_INPUT_TOKENS = _i("LLM_MAX_INPUT_TOKENS", 4000)
LLM_ALERT_MIN_CONF = _f("LLM_ALERT_MIN_CONF", 0.60)
LLM_ANALYST_MIN_SCORE = _f("LLM_ANALYST_MIN_SCORE", 0.0)
LLM_USE_PROXY = os.getenv("LLM_USE_PROXY", "false").lower() == "true"
LLM_IGNORE_PROXY = os.getenv("LLM_IGNORE_PROXY", "true").lower() == "true"

# Macro watcher (not wired here, but env-ready)
MACRO_WINDOWS_STR = os.getenv("MACRO_WINDOWS", "12:30,14:00")
MACRO_WINDOWS = [w.strip() for w in MACRO_WINDOWS_STR.split(",") if w.strip()]
MACRO_WINDOW_MINUTES = _i("MACRO_WINDOW_MINUTES", 10)
MACRO_RVOL_MIN = _f("MACRO_RVOL_MIN", 1.3)
MACRO_INTERVAL_SEC = _i("MACRO_INTERVAL_SEC", 60)
...
PG_DD_SLOW_BPS = _f("PG_DD_SLOW_BPS", -5.0)
POSTURE_MAX_AGE_MIN = _i("POSTURE_MAX_AGE_MIN", 30)
PG_PERSIST_K = _i("PG_PERSIST_K", 5)
POSTURE_GUARD_INTERVAL = _i("POSTURE_GUARD_INTERVAL", 30)

# Trend score params
TS_INTERVAL = _i("TS_INTERVAL", 30)
TS_ENTRY = _f("TS_ENTRY", 0.65)
TS_EXIT = _f("TS_EXIT", 0.35)
TS_PERSIST = _i("TS_PERSIST", 2)
